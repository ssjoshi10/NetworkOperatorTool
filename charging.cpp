...
{
	struct date from_date = {0,0,0}, to_date = {0,0,0}, curr_dt = {0,0,0};
	...
	if(val_info.month == curr_dt.month && val_info.yr == curr_dt.year)
	{
		return 0;
	}
	...
	{
		from_date.month = 12U;
		from_date.year = curr_dt.year - 1U;
	}
	else
	{
		from_date.month = curr_dt.month - 1U;
