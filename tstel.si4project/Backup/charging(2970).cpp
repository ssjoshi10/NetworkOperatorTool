...
void charging::rchg_cu(ull msin, struct plan_info plan_details, string str,database *db_obj)
{
	...
	time_t now = time(0);
	tm *ptr = localtime(&now);							//getting today's date
...
/*****************************************************************************************************
Function Name: int calculate_validity(date dt1,date dt2)
...
int charging::calculate_validity(date dt1,date dt2) 
{
	long int n1 = (dt1.y)*YEAR + dt1.d;  
	for (int i = 0U; i < dt1.m - 1U; i++) 
		n1 += Mdays[i]; 

	long int n2 = dt2.y*YEAR + dt2.d; 
	for (int i = 0U; i < dt2.m - 1U; i++) 
		n2 += Mdays[i]; 
...
/*****************************************************************************************************
Function Name: void rchg_date(int *d, int *m, int *y, int days)
...
void charging::rchg_date(int *d, int *m, int *y, int days)
{
	*d = *d + days;
	while(*d > Mdays[*m - 1U])
	{
...
/*****************************************************************************************************
Function Name: void get_bal_cu(ull msin,database *db_obj)
...
void charging::get_bal_cu(ull msin,database *db_obj)
{
	struct date lst_rchg_dt = {0,0,0}, curr_dt = {0,0,0}, nxt_rchg_dt = {0,0,0};
	time_t now=time(0);
	tm *ptr = localtime(&now);								//getting today's date//
	curr_dt.d = ptr->tm_mday;
	curr_dt.m = 1U + ptr->tm_mon;
	curr_dt.y = CAL_BASE + ptr->tm_year;

	//Getting data from database//
	struct validity_info *sub_info = db_obj->get_validity_db(msin, "prepaid");

	if(sub_info == NULL)
	{
		cout<<"MSIN is in blacklist (or) MSIN is Invalid \nCan't display balance"<<endl;
	}
	else
	{
		lst_rchg_dt.d = sub_info->date;
		lst_rchg_dt.m = sub_info->month;
		lst_rchg_dt.y = sub_info->yr;

		//getting validity//
		int diff = calculate_validity(lst_rchg_dt,curr_dt);
		
		//getting next recharge date//
		nxt_rchg_dt.d = curr_dt.d;
		nxt_rchg_dt.m = curr_dt.m;
		nxt_rchg_dt.y = curr_dt.y;
		rchg_date(&nxt_rchg_dt.d, &nxt_rchg_dt.m, &nxt_rchg_dt.y, (sub_info->no_of_days)-diff);
				
		//Printig the details on stdout//
		cout<<"MSIN: "<<msin<<endl;
		cout<<"Available balance is: "<<sub_info->amt;
		cout<<"\nvalidity is: "<<sub_info->no_of_days-diff<<" days"<<endl;
		cout<<"Next recharge date is: "<<nxt_rchg_dt.d<<"/"<<nxt_rchg_dt.m<<"/"<<nxt_rchg_dt.y<<endl;
	}
}

/*****************************************************************************************************
Function Name: void generate_bill(ull msin,validity_info st)
...
void charging :: generate_bill(ull msin,validity_info val_info)
{
	ofstream myfile;
	myfile.open ("bill.csv");		//creating file//

	//Storing the data into file//
	myfile << "Postpaid Bill\n";
	myfile << "msin,"<<msin << "\n";
	myfile << "imei,"<<val_info.imei <<"\n";
	myfile << "Amount,"<<val_info.amt <<" Rs\n";
	myfile << "Date,"<<val_info.date<<"/"<<st.month<<"/"<<st.yr<<"\n";
	myfile.close();
}

/*****************************************************************************************************
Function Name: void gen_bill_cu(ull msin,database *db_obj)
...
void charging::gen_bill_cu(ull msin,database *db_obj)
{
	//Getting data from database//
	struct validity_info *sub_info = db_obj-> get_bill_db(msin, "postpaid");
	if(sub_info == NULL)
	{
		cout<<"MSIN is in blacklist (or) MSIN is Invalid \nCan't gernerate bill"<<endl;
	}
	else
	{	
		struct validity_info val_info = {0,0,0,0,0,0};
		...
		cout<<"MSIN: "<<msin<<endl;
		cout<<"Total bill amount for last month is: "<<sub_info->amt;

		//Generating bill//
		generate_bill(msin,val_info);
		cout<<"\nBill generated\n";
	}
}
